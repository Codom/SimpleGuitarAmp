...
export const clap_entry = c.clap_plugin_entry{ .clap_version = .{ .major = 1, .minor = 1, .revision = 6 }, .init = init_plugin, .deinit = deinit_plugin, .get_factory = get_factory };
...
pub const PluginDescriptor = c.clap_plugin_descriptor{
    .clap_version = .{ .major = c.CLAP_VERSION_MAJOR, .minor = c.CLAP_VERSION_MINOR, .revision = c.CLAP_VERSION_REVISION },
    .id = "zig.HelloCLAP",
    .name = "HelloCLAP",
...
fn create_plugin(
    ...
    _ = factory;
    if (!c.clap_version_is_compatible(host.*.clap_version) or
    ...
    log_args.host = host;
    log_args.log = @ptrCast(@alignCast(host.*.get_extension.?(host, &c.CLAP_EXT_LOG)));
    ...
    ret.sample_rate = 0;
    ret.clap_plugin.plugin_data = @ptrCast(ret);
...
pub fn log(
    ...
    if (log_args.log) |l| {
        const string_sl = std.fmt.allocPrintZ(c_allocator, format, args) catch return;
        defer c_allocator.destroy(string_sl.ptr);
        const string: [*c]const u8 = string_sl.ptr;
